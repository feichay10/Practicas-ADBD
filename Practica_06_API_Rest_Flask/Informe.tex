...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{img/create_book.png}
  \caption{Insertar registros}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.27]{img/added_book.png}
...
llamada \emph{delete()} en la que se realiza la siguiente consulta para borrar una entrada de la tabla \emph{books}, aqui
el codigo en python:
\lstset{style=mystyle}
\lstinputlisting[language=sql]{code/delete_books.py}

Este codigo en python lo que hace es la siguiente consulta en sql:
\lstset{style=mystyle}
